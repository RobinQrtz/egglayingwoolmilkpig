...
class DependencyDecoder(object):
    ...
    def parse_proj(self, scores):
        ...
        if len(list(filter(lambda x: x ==0, heads))) > 1:
            print(heads)
        value_proj = 0.0
    ...
    def parse_nonproj(self, scores):
        ...
        # horrible functions to rearrange matrix
        ltr = lambda m: np.hstack((m[:,1:], np.array([m[:,0]]).T))
        utd = lambda m: np.vstack((m[1::], m[:1:]))
        dtu = lambda m: np.vstack((m[-1::], m[:-1:]))
        rtl = lambda m: np.hstack((np.array([m[:,-1]]).T,m[:,:-1]))
        #scores_copy = rtl(dtu(scores))
        ...
        for key in final_edges.keys():
            ...
            pr = final_edges[key]
            heads[ch] = pr
        matrix = np.zeros((nr,nc), int)
        for m,h in enumerate(heads[1:]):
            matrix[h,m+1] = 1
        #print(matrix)
        #matrix = ltr(utd(matrix))
        if self.verbose:
            self.verbose = False
            return matrix
        elif len(list(filter(lambda x: x ==0, heads))) > 1:
            print(heads)
            self.verbose = True
            self.parse_nonproj(scores)
        return matrix
